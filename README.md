...
- 夜班交接前在主导航“位置盘点”核对冷藏点实物：选择冷藏位置后逐行扫描容器标签并提交（`POST /api/locations/{location_id}/inventory-checks`，载荷为 `checked_by` 与非空、去重、去首尾空白的 `labels`）。服务端在同一事务先 `SELECT … FOR UPDATE` 锁定位置，再锁定账面记录在该位置的全部 `active` 容器形成快照，随后以标签集合比对：在本处记录命中为已匹配（matched），账面在本处但未扫到为账面缺失（missing），标签属于其他位置的可流转容器为错放到此处（misplaced），系统中无对应可流转容器标签为未知标签（unknown）。盘点记录（`location_inventory_checks` 及 `location_inventory_check_items`）不可变，保存提交人、服务端时间、四个分类计数与逐行明细；错放行携带系统记录位置编码与名称，供人员按原交接流程纠正。盘点只形成核对证据：不移动容器、不改批次处置、不写交接/测温记录，也不产生责任链事件。重复（忽略大小写）或空白标签返回 `INVALID_INVENTORY_LABELS` 且不开事务、不落库；位置不存在返回 `LOCATION_NOT_FOUND`；非冷藏位置返回 `LOCATION_NOT_COLD_STORAGE`。已封存容器退出可流转快照：扫到旧标签按未知标签处理。提交响应含本次四类分组明细与该位置本次之前的最近记录（`GET /api/locations/{id}/inventory-checks`，不可变记录重放不会混入后来的盘点）；前端逐行校验原始扫描：任何位置（标签之间、开头或末尾）出现空白行都会在请求前提示“标签清单无效（第 N 行为空白标签）”并保留原文、不发请求、不生成盘点记录——每个标签必须恰好占一行，扫描枪多余的回车需要先删除；断网或提交失败时标签与盘点人保留在表单中，成功后清空已扫标签但保留盘点人以便连续盘点。
