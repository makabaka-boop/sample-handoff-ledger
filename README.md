...
11. 打开批次详情，在 `active` 容器上点“补录人工测温”，填写 5°C、测量人、测量时间后提交：立即看到“正常”判定，批次仍为 active，时间线只多一条 `temperature_recorded`。再对同一容器补录 9.5°C（对 2–8°C 温区）：判定为“越界”，批次进入 `review`，容器位置与离柜计时不变，时间线同样只有一条对应事件；批次详情按测量时间倒序列出温度、判定、测量人与时间。对已封存容器提交得到 `CONTAINER_ALREADY_REPLACED`；把测量时间改成未来或批次创建之前得到 `INVALID_OBSERVED_AT`，失败后表单温度、人员、时间、备注全部保留。
12. 打开“位置盘点”，选择冷藏冰箱，逐行录入现场扫到的容器标签与盘点人后提交：看到已匹配、账面缺失、错放到此处、未知标签四组计数与明细；错放行显示“系统记录位置”，但批次详情中容器位置、批次处置和责任链均无变化，时间线没有盘点事件。对非冷藏位置无法提交（页面只列冷藏位置，接口返回 `LOCATION_NOT_COLD_STORAGE`）；提交重复或空白标签得到 `INVALID_INVENTORY_LABELS` 且数据库中没有任何盘点记录。断网时标签与盘点人保留，恢复后可原样重试；成功后展示本次结果与该位置的最近盘点记录。
